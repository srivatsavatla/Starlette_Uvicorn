...
client = MongoClient('mongodb+srv://srivatsav:srivatsav@cluster0.0pmprnp.mongodb.net/')
db = client['Courses']
...
def test_get_course_by_id_exists():
    response = client.get("/courses/6757a774d83248c7b7831b05")
    assert response.status_code == 200
    course = response.json()
    # get the course from the database
    course_db = db.courses.find_one({'_id': ObjectId('6757a774d83248c7b7831b05')})
...
def test_get_chapter_info():
    response = client.get("/courses/6757a774d83248c7b7831b05/1")
    assert response.status_code == 200
...
def test_get_chapter_info_not_exists():
    response = client.get("/courses/6757a774d83248c7b7831b05/990")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Chapter not found'}

def test_rate_chapter():
    course_id = "6757a774d83248c7b7831b05"
    chapter_id = "1"
    rating = 1
...
def test_rate_chapter_not_exists():
    response = client.post("/courses/6757a774d83248c7b7831b05/990/rate", json={"rating": 1})
    assert response.status_code == 404
